...
class SmoothDifferentialDriveVehicle : public Vehicle
{
private:
    // Properties for path control (using point control method)
    double eps_path = 1.0; // Initial epsilon for controlling a point to a path
    double eps_path_min = 0.2; // Minimum value for eps_path
    bool use_dim_eps = false; // true => use a diminishing epsilon, false will use a constant epsilon of eps_path
    Eigen::Matrix3d K_point_ctrl; // Feedback matrix for point control, used with feedback on 
                     // K_point_ctrl*(q - q_des), where q is the
                     // position and velocity of a point
                     
    // Properties for point-method velocity control
    double eps_vel = 0.2; // Epsilon to be used for velocity control
    Eigen::Matrix3d K_point_vel; // Feedback matrix for point control of velocity, used with feedback on 
                     // K_point_vel*(q - q_des), where q is the velocity of a point                     
                     
    // Properties for velocity control 
    Eigen::Matrix3d K_vel; // Feedback matrix for velocity control where the state is the 
            // translational and rotational velocities
            
            
    
    // Properties for vector field orientation control 
    Eigen::Matrix3d K_orient; // Feedback matrix for velocity control where the state is the 
            // translational and rotational velocities
            
    // Gains for control
    double k_wd = 2; // Gain for the desired rotational velocity
    double vd_field_max = 1; // Maximum desired velocity from a vector field
...
public:
    SmoothDifferentialDriveVehicle();
    ~SmoothDifferentialDriveVehicle();
};

SmoothDifferentialDriveVehicle::SmoothDifferentialDriveVehicle(/* args */)
    : Vehicle(SmoothDifferentialDrive), std::vector<double>(5,0))
{
...
SmoothDifferentialDriveVehicle::~SmoothDifferentialDriveVehicle()
{
}
