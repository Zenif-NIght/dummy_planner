...
class BetterUnicycleVehicle : public Vehicle
{
private:
    // Properties for path control (using point control method)
    double m_eps_path = 1.0; // Initial epsilon for controlling a point to a path
    double m_eps_path_min = 0.2; // Minimum value for eps_path
    bool m_use_dim_eps = false; // true => use a diminishing epsilon, false will use a constant epsilon of eps_path
    vector<double> m_K_point_ctrl; // Feedback matrix for point control, used with feedback on 
                    // K_point_ctrl*(q - q_des), where q is the
                    // position and velocity of a point
                    
    // Properties for point-method velocity control
    double m_eps_vel = 0.2; // Epsilon to be used for velocity control
    Matrix2d m_K_point_vel; // Feedback matrix for point control of velocity, used with feedback on 
                    // K_point_vel*(q - q_des), where q is the velocity of a point                     
                    
    // Properties for velocity control 
    vector<double> m_K_vel; // Feedback matrix for velocity control where the state is the 
            // translational and rotational velocities
            
    // Gains for control
    double m_k_wd = 2; // Gain for the desired rotational velocity
    double m_vd_field_max = 5; // Maximum desired velocity from a vector field
    ...
    int v_ind = 3; // translational velocity index
    int w_ind = 4; // angular velocity index (omega)
    
    // Input indices
    int u_a_ind = 1; // Translational acceleration input
...
//         Eigen::Vector2d (*g_function)(int, const vector<double> &, const vector<double>&),
